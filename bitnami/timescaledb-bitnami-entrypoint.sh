...
then
    POSTGRESQL_SHARED_PRELOAD_LIBRARIES="citus,timescaledb,zombodb,pg_repack,pgautofailover,hll,pg_cron"
else
    POSTGRESQL_SHARED_PRELOAD_LIBRARIES="citus,timescaledb,zombodb,pg_repack,pgautofailover,hll,pg_cron,$POSTGRESQL_SHARED_PRELOAD_LIBRARIES"
fi
